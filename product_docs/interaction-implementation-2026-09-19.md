...
The new server schema/routes and native client require coordinated release. On September 20, the integrated source was pushed to `main`, build 2609202041 was accepted by Apple, and the matching backend deployment became live. Apple processing and Internal TestFlight availability still require verification after browser sign-in; see [the release record](native-release-2609202041.md). No live-provider end-to-end result is claimed. The local lifecycle gate assumes a single serving backend process; several concurrent processes need shared cancellation coordination.
...
## September 21 gesture regression repair

The card-wide simultaneous hold added for touch-down feedback blocked real feed swipes. Removing that recognizer restores scrolling; button press feedback remains on individual controls. A native XCTest reproduced the failure before the change and passes afterward. The original illustrated archive was superseded before upload; [release 2609210445](native-release-2609210445.md) records the combined release and validation.
