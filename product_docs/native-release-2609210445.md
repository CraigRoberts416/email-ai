...
- Native gesture suite on iOS 26.5 and iOS 26.4.1: all four tests passed on each (eight total). Covers card-origin swipe, return-to-top followed by another swipe, opening/closing a thread, tab return, and scrolling while the illustrated refresh is working.
- FeedStore production integration: 156 checks passed.
- The first iOS 26.4.1 test runner stalled before execution. Restarting only the dedicated test Simulator recovered it; the complete suite then passed.
...
The retained `FeedGestures` scheme runs genuine touch gestures; programmatic `scrollTo` is insufficient to catch this failure. See [test instructions](../apple/DecisionInboxUITests/README.md). Baseline failure: `/tmp/di-gesture-before.xcresult`; corrected suites: `/tmp/di-gesture-regression.xcresult` and `/tmp/di-gesture-regression-264-retry.xcresult`. Result bundles contain native event traces and screen recordings. Physical-device confirmation remains distinct from Simulator verification.

## Distribution

Release archive `2609210445` passed. Its bundle identifier/version, artwork checksum, bundled license and absence of DEBUG sample routes/UI-test bundles were verified. The command-line upload was rejected before transfer with `Failed to Use Accounts`: Xcode could not find an account with App Store Connect access for team `48X38356RX`. **Xcode Organizer successfully uploaded 1.0 (2609210445) at 04:47 EDT.** Its confirmation and status log both identify this exact build. [Upload receipt](../tools/motion-lab/native-qa/scroll-regression/upload-2609210445.jpg). Implementation commit `427f495` is pushed to `main`. Apple processing and Internal-group Testing status remain unverified: Chrome reports an open extension UI blocking App Store Connect, and the user has been asked to close it. No tester membership was changed. The corrected archive is retained at `~/Library/Developer/Xcode/Archives/2026-09-21/DecisionInbox 2026-09-21 04.45.xcarchive`. The backend remains the already-live `a6e9043`; this repair and the illustrated layer do not change the server contract.
