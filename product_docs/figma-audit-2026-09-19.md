...
Final native distribution and backend evidence is recorded separately in [release 2609191136](./native-release-2609191136.md). The release is available for internal testing; historical import is advancing but not claimed exhaustive. Intermediate verification notes below retain their timing and do not replace that final record.
...
- The implementation task reports successful live checks: two scrolled-past cards changed to read and retained their order; Feed-tab return removed read cards; opening/closing an email preserved its read card; pull-to-refresh removed it. A later isolated Home-and-app-icon return, without tab switching, removed the read card while retaining its unread neighbors and showing new arrivals at the head. These reports support the tested session boundaries. The later complete-backfill result is recorded below.
...
The implementation task verified **No more emails.** and opening old posts with the isolated DEBUG `sampleFeedZero` fixture, plus an off preference persisting after Settings reopened; **See old posts** was restored to On afterward. Real Gmail was retained without bulk read changes. These checks support the completion UI and preference specimens, not a claim that the real mailbox reached zero. Inline CID image recovery and backend/native release were still under retest at that checkpoint; their later results are recorded in the release report rather than inferred from successful Figma rendering.
