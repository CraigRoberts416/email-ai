...
* Each section header has its own right-aligned **X LEFT** count; both the day label and LEFT are uppercase. The total includes eligible unread mail not yet loaded into cards. A qualifying scroll-past immediately animates its displayed decrement while a bounded serial read queue obtains server confirmation. Failure animates the provisional decrement back up. Keep the last verified section baseline visible during temporary revalidation, tab changes, app return and cold launch. Persist the baseline on device. When a new session crosses midnight in the same time zone, roll its known counts into the new day bands; a changed time zone, backwards clock change or baseline older than seven days requires fresh counts. New arrivals and server verification reconcile the retained baseline. This responsive display is not authority for badge totals or completion. Read cards may remain below a zero-count header until the session resets.
