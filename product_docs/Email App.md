...
The approved app-wide interaction implementation adds durable device-local saved snapshots, drafts and discussion; separate send jobs and valid per-action Undo; and persistent unsubscribe activity with evidence and a fresh-page human handoff. Search covers loaded feed/saved content; People identifies its current first mailbox. Disconnect confirms stopped server access before local removal and discloses retained server mail. The [implementation record](./interaction-implementation-2026-09-19.md) is authoritative for these behaviors and their validation limits. The interaction backend is live and native build 2609202041 was accepted by Apple. The subsequent illustrated layer adds three production Rive artboards; [release 2609202100](./native-release-2609202100.md) is the authoritative distribution/verification record. Upload acceptance and tester availability are tracked separately.
...
* A fresh session begins when the app returns after being left, when the user switches away from Feed and returns, or when the user pulls to refresh. At that boundary, remove confirmed-read posts and accepted reads still saving, then rebuild the incoming view from remaining unread mail. Proven reads are durably recorded before delivery; navigating away does not cancel them. Retry interrupted writes after app relaunch or reconnection. If delivery fails, restore the provisional count and retain the email for retry and the next incoming session. A detail-view appearance callback is not a session boundary.
* Card-wide touch-down or drag recognizers must not prevent the feed’s native pan. Press feedback belongs to individual controls. Verify real swipes after card navigation, tab changes, return-to-top and during refresh; programmatic scroll tests do not establish gesture recognition.
* New mail arriving during a session uses the existing new-post bubble. It is held pending until the user admits it, rather than silently inserted into the reading position. Bubble and endpoint admission insert the batch before a one-shot scroll to the permanent top of the updated feed. Both must leave subsequent manual scrolling free; programmatic navigation never creates read evidence.
...
* Each section header has its own right-aligned **X LEFT** count; both the day label and LEFT are uppercase. The total includes eligible unread mail not yet loaded into cards. A qualifying scroll-past immediately animates its displayed decrement while a bounded serial read queue obtains server confirmation. Failure animates the provisional decrement back up. Keep the last verified section baseline visible during temporary revalidation, tab changes, app return and cold launch. Persist the baseline on device. Reuse it while the calendar day and time zone are unchanged. A new day or changed time zone requires fresh section counts: an active session can span midnight, so its aggregate TODAY count cannot safely be split across new day bands. New arrivals and server verification reconcile the retained baseline. This responsive display is not authority for badge totals or completion. Read cards may remain below a zero-count header until the session resets.
...
**People tab notification badge (September 20, 2026):** the native People tab shows its unread-conversation count, using the same count as the People masthead. Multiple unread emails in one conversation contribute one. While history is importing, use known loaded unread conversations; when the inventory is complete, use the verified aggregate, including unloaded conversations. Hide the badge at zero. Load the badge without requiring a visit to People, refresh on new-mail/read reconciliation and app refresh, and recheck every 30 seconds while the app is active. Preserve known state on refresh failure. Visiting the People tab alone does not clear unread mail. The app-icon badge remains the separate all-account unread-email total.
